...
const ExploreMovies = () => {
  ...
  const fetchMovies = useCallback(
    async (pageNum, append = false, searchQuery = "", filterParams = {}) => {
      try {
        let data;
        if (searchQuery) {
          data = await fetchMoviesWithPagination("/movies/search", {
            query: searchQuery,
            page: pageNum,
          });
        } else {
          data = await fetchMoviesWithPagination("/movies/discover", {
            page: pageNum,
  ...
  useEffect(() => {
    ...
    fetchMovies(1, false, search, filterParams).finally(() => setLoading(false));
  ...
  const handleLoadMore = async () => {
    ...
    await fetchMovies(nextPage, true, search, filterParams);
    setPage(nextPage);
  ...
  const years = Array.from({ length: currentYear - 1990 + 1 }, (_, i) => currentYear - i);
   ...
   return (
     <section className="py-5 px-6 min-h-screen">
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4 justify-items-center px-4">
          ...
          {page < totalPages && (
            <div className="flex justify-center mt-8">
              <button
                ...
                className="px-8 py-3 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 text-white font-semibold rounded-lg transition-colors duration-300"
              >
